...
public class BankPaymentGateway implements PaymentGateway {
  private static final Logger logger = LoggerFactory.getLogger(BankPaymentGateway.class);
  ...
  public PaymentGatewayResponse process(PaymentGatewayRequest request) {
    logger.info("CLIENT: Bank is processing transfer for order: {}", request.getOrderId());

    if (request.getAmount().compareTo(LIMIT) > 0) {
      return new PaymentGatewayResponse(false, "BANK: Transaction failed - Amount exceeds 10M limit");
    }

    return new PaymentGatewayResponse(true, "BANK: Bank transfer successful");
  }
}